...
from psd2.evaluation import (
    ...
    PrwQueryEvaluatorP,
    CuhkQueryEvaluatorP,
    PrwAPSQueryEvaluator,
    CuhkAPSQueryEvaluator,
    PrwAPSQueryEvaluatorP,
    CuhkAPSQueryEvaluatorP,
)
import re
...
def build_evaluator(cfg, dataset_name, output_folder=None):
    ...
    query_eval_kwargs = dict(
        ...
        search_top_pants_soft_rerank_uncertain_margin=cfg.TEST.SEARCH_TOP_PANTS_SOFT_RERANK_UNCERTAIN_MARGIN,
        search_top_pants_soft_rerank_score_band=cfg.TEST.SEARCH_TOP_PANTS_SOFT_RERANK_SCORE_BAND,
        search_top_pants_soft_rerank_max_delta=cfg.TEST.SEARCH_TOP_PANTS_SOFT_RERANK_MAX_DELTA,
        cuhk_search_top_pants_positive_only=cfg.TEST.CUHK_SEARCH_TOP_PANTS_POSITIVE_ONLY,
        coarse_topk=cfg.TEST.COARSE_TOPK,
        reuse_eval_cache=cfg.TEST.REUSE_EVAL_CACHE,
        save_query_cache=cfg.TEST.SAVE_QUERY_CACHE,
    )
    aps_eval_kwargs = dict(
        aps_match_mode=cfg.TEST.APS_MATCH_MODE,
        **query_eval_kwargs,
    )
    if "SCHEMA_PATH" in cfg.PERSON_SEARCH.REID:
        aps_eval_kwargs["schema_path"] = cfg.PERSON_SEARCH.REID.SCHEMA_PATH
    if evaluator_type == "det":
        ...
        use_aps_protocol = str(cfg.TEST.SEARCH_PROTOCOL).lower() == "aps"
