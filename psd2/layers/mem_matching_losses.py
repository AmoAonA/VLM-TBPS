...
def _cast_memory_for_backward(memory, grad_output):
    if memory.dtype == grad_output.dtype:
        return memory
    return memory.to(dtype=grad_output.dtype)
...
class LabeledMatching(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            grad_feats = grad_output.mm(_cast_memory_for_backward(lookup_table, grad_output))
        _update_table_sync(
            lookup_table,
            pid_labels,
            features,
...
class UnlabeledMatching(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_feats = None
        if ctx.needs_input_grad[0]:
            grad_feats = grad_output.mm(_cast_memory_for_backward(queue.data, grad_output))
        # print(tail)
