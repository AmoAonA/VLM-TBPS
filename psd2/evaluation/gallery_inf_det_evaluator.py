...
class InfDetEvaluator(DatasetEvaluator):
    def __init__(
        ...
        self._cached_inference_ready = False
        self._cached_det_result = None
        # (image name,torch concatenated [boxes, scores, reid features])
        self.inf_results = {}
        self.inf_group_feats = {}
        self.inf_group_valids = {}
        self.inf_group_labels = {}
        self.cerberus_group_score_weight = None
    ...
    def reset(self):
        ...
        if self.reuse_eval_cache:
            ...
            if os.path.exists(cache_path):
                ...
                self.inf_results = cached.get("infs", {})
                self.inf_group_feats = cached.get("inf_group_feats", {})
                self.inf_group_valids = cached.get("inf_group_valids", {})
                self.inf_group_labels = cached.get("inf_group_labels", {})
                self.cerberus_group_score_weight = cached.get(
                    "cerberus_group_score_weight",
                    1.0 if len(self.inf_group_feats) > 0 else 0.0,
                )
                self.gallery_gts = cached.get("gts", {})
                self.gallery_attrs = cached.get("gt_attrs", {})
                self.gallery_fnames = cached.get("gt_fnames", {})
                self._cached_det_result = cached.get("det_result", {})
                self._cached_inference_ready = True
                return
        self.inf_results = {}
        self.inf_group_feats = {}
        self.inf_group_valids = {}
    ...
    def supports_cached_inference(self):
        return bool(self._cached_inference_ready)

    def process(self, inputs, outputs):
        if self._cached_inference_ready:
            return
        """
        Args:
            inputs:
        ...
        for bi, in_dict in enumerate(inputs):
            ...
            if hasattr(gt_instances, "gt_attributes"):
                self.gallery_attrs[gt_img_name] = copy.deepcopy(gt_instances.gt_attributes)
            else:
                self.gallery_attrs[gt_img_name] = [{} for _ in range(len(gt_instances))]
            self.gallery_fnames[gt_img_name] = gt_instances.file_name
            if pred_instances.has("cerberus_group_score_weight") and len(pred_instances.cerberus_group_score_weight) > 0:
                self.cerberus_group_score_weight = float(pred_instances.cerberus_group_score_weight[0].item())
            # save inf results
            ...
            if scores_t.shape[0] > self.topk:
                ...
                out_group_feats_t = (
                    pred_instances.cerberus_group_feats[idx]
                    if pred_instances.has("cerberus_group_feats")
                    else None
                )
                out_group_valid_t = (
                    pred_instances.cerberus_group_valid[idx]
                    if pred_instances.has("cerberus_group_valid")
                    else None
                )
                out_group_labels_t = (
                    pred_instances.cerberus_group_labels[idx]
            ...
            if out_group_feats_t is not None:
                self.inf_group_feats[gt_img_name] = out_group_feats_t
            if out_group_valid_t is not None:
                self.inf_group_valids[gt_img_name] = out_group_valid_t
            if out_group_labels_t is not None:
                self.inf_group_labels[gt_img_name] = out_group_labels_t
    ...
    def evaluate(self):
        if self._cached_inference_ready:
            return copy.deepcopy(self._cached_det_result if self._cached_det_result is not None else {})
        if self._distributed:
            ...
            for rk in save_ranks:
                ...
                g_attrs = comm.gather(self.gallery_attrs, dst=rk)
                inf_rts = comm.gather(self.inf_results, dst=rk)
                group_feats = comm.gather(self.inf_group_feats, dst=rk)
                group_valids = comm.gather(self.inf_group_valids, dst=rk)
                group_labels = comm.gather(self.inf_group_labels, dst=rk)
                ...
                if len(group_feats) > 0:
                    for feats in group_feats:
                        save_group_feats.update(feats)
                if len(group_valids) > 0:
                    for valids in group_valids:
                        save_group_valids.update(valids)
                if len(group_labels) > 0:
                    for labels in group_labels:
                        save_group_labels.update(labels)
                if len(g_gts) > 0:
                    for gts in g_gts:
                        save_gts.update(gts)
                if len(g_attrs) > 0:
                    for attrs in g_attrs:
                        save_attrs.update(attrs)
                if len(f_gts) > 0:
                    for fs in f_gts:
                        save_gtfs.update(fs)
            ...
            save_group_feats = self.inf_group_feats
            save_group_valids = self.inf_group_valids
            save_group_labels = self.inf_group_labels
            save_gts = self.gallery_gts
            save_attrs = self.gallery_attrs
        ...
        save_dict = {
            ...
            "infs": save_rts,
            "gt_fnames": save_gtfs,
            "inf_group_feats": save_group_feats,
            "inf_group_valids": save_group_valids,
            "inf_group_labels": save_group_labels,
            "cerberus_group_score_weight": (
                self.cerberus_group_score_weight
                if self.cerberus_group_score_weight is not None
                else (1.0 if len(save_group_feats) > 0 else 0.0)
            ),
            "det_result": None,
        }
        ...
        save_dict["det_result"] = copy.deepcopy(det_result)
        torch.save(save_dict, save_path)
