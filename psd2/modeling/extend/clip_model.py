...
class CLIP(nn.Module):
    ...
    def encode_text(self, text,ckpt=False):
        ...
        return x
    def encode_text_zero_dropout(self, text,ckpt=False):
        prev_dropout=[]
        for resblock in self.transformer.resblocks:
            prev_dropout.append(resblock.attn.dropout)
            resblock.attn.dropout=0.0
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        ...
        # x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection
        x = x @ self.text_projection
        for resblock,d in zip(self.transformer.resblocks,prev_dropout):
            resblock.attn.dropout=d
        return x
